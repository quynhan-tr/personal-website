import React, { useState } from "react";
import EducationCard from "./EducationBar";
import { education } from "../data/education";
import { Playfair_Display } from "next/font/google";

const playfair = Playfair_Display({
  subsets: ["latin"],
  weight: ["400", "500", "600", "700"],
});

export default function EducationSection() {
  const [openIndex, setOpenIndex] = useState<number | null>(null);
  return (
    <section id="education" className="max-w-7xl mx-auto px-4 lg:px-8 pb-10 md:pb-15 pt-10 md:pt-16 lg:pt-20 flex flex-col lg:flex-row gap-8 lg:gap-12 items-start">
      {/* Heading on the left */}
      <div className="lg:w-1/3 w-full mb-6 lg:mb-0 lg:sticky lg:top-20 z-10">
        <h2 className={`text-3xl md:text-4xl lg:text-5xl text-white leading-tight font-normal ${playfair.className}`}>
          EDUCATION
        </h2>
      </div>
      {/* Cards on the right */}
      <div className="flex-1 grid grid-cols-1 w-full">
        {education.map((edu, idx) => (
          <EducationCard
            key={idx}
            {...edu}
            open={openIndex === idx}
            onOpen={() => setOpenIndex(idx)}
            onClose={() => setOpenIndex(null)}
          />
        ))}
      </div>
    </section>
  );
}
